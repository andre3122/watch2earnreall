...
module.exports = async (req, res) => {
  ...
  const { rows: list } = await sql`
    SELECT referred_user_id AS id, to_char(created_at,'YYYY-MM-DD') AS joined
    FROM referrals WHERE user_id=${a.user.id}
    ORDER BY created_at DESC
    LIMIT 25
  `;

  // You can enrich "name" by joining users table; keep simple
  res.json({ count, list: list.map(r => ({ name: `User ${r.id}`, joined: r.joined })) });
};
