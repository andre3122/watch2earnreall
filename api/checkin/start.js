...
module.exports = async (req, res) => {
  ...
  // read user streak + last_checkin
  const { rows: ur } = await sql`SELECT streak, last_checkin FROM users WHERE id=${a.user.id} LIMIT 1`;
  const streak = Number(ur?.[0]?.streak || 0);
  const last = ur?.[0]?.last_checkin ? String(ur[0].last_checkin).slice(0,10) : null;
  const today = todayStr();

  if (last === today) return res.status(200).json({ ok:true, allowed:false, reason:"ALREADY" });
  if (streak >= 9)   return res.status(200).json({ ok:true, allowed:false, reason:"COMPLETE" });

  const day = streak + 1;
  const reward = CHECKIN_REWARDS[day-1];
  const token = crypto.randomBytes(16).toString('hex');
  const taskId = `checkin:${day}`;
  ...
  await sql`
    ...
    VALUES (${a.user.id}, ${taskId}, ${token}, ${reward}::numeric, 'pending')`;
