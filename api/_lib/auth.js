// api/_lib/auth.js — strict by default; permissive opsional
const { validateInitData, parseInitData } = require("./telegram");
const { getUserOrCreate } = require("./db"); // asumsi sudah ada
...
async function authFromHeader(req) {
  ...
  // 1) initData Telegram
  if (raw && botToken && typeof validateInitData === "function") {
  ...
  // 3) permissive fallback (sementara)
  if (PERMISSIVE) {
